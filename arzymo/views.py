...
from arzymo.serializers import *
...
def create_user(request):
    ...
    if serializer.is_valid():
        ...
        return Response({'message': 'User created', 'error': False, 'token': token.key})
    else:
...
def login(request):
    ...
    token, _ = Token.objects.get_or_create(user=user)
    return Response({"token": token.key, 'user_id': user.pk, 'error': False})
